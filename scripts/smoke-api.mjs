...
await build({
  entryPoints: ["api/brief.ts", "api/health.ts", "api/paystack.ts"],
  bundle: true,
  platform: "node",
  format: "esm",
  target: "node20",
...
const paystack = (await import(path.join(root, "dist-ssr-api/paystack.js"))).default;
...
// Paystack verify: unconfigured -> 503 {configured:false}; wrong method -> 405
{
  const un = mockRes();
